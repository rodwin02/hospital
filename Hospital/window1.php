...
<body>
    ...
    <div class="normal-content" >
        <div class="home-cont">
           ...
           <div class="reset" onclick="resetTicketNumber()">RESET TICKET NUMBER</div>
        </div>
    </div>
    ...
    <div class="priority-content" style="display: none;">
        <div class="home-cont">
            ...
            <div class="button-cont">
                <div class="previous-cont">
                    <div class="previous" onclick="previousPriorityTicket()">PREVIOUS</div>
            ...
            <div class="reset" onclick="resetPriorityTicketNumber()">RESET TICKET NUMBER</div>
        </div>
    </div>
</body>
</html>
...
<script>
    ...
    function updateTicketNumber() {
        ...
        ticketNumberElement.textContent = `IP${currentTicketNumber.toString().padStart(3, '0')}`;
        nextTicketNumberElement.textContent = `IP${(currentTicketNumber + 1).toString().padStart(3, '0')}`;
        previousTicketNumberElement.textContent = `IP${(currentTicketNumber - 1).toString().padStart(3, '0')}`;
    }
    ...
    function updatePriorityTicketNumber() {
        const priorityTicketNumberElement = document.querySelector('.priority-content .ticket-number');
        const nextPriorityTicketNumberElement = document.querySelector('.priority-content .next-num');
        const previousPriorityTicketNumberElement = document.querySelector('.priority-content .previous-num');

        priorityTicketNumberElement.textContent = `PRIP${currentPriorityTicketNumber.toString().padStart(3, '0')}`;
        nextPriorityTicketNumberElement.textContent = `PRIP${(currentPriorityTicketNumber + 1).toString().padStart(3, '0')}`;
        previousPriorityTicketNumberElement.textContent = `PRIP${(currentPriorityTicketNumber - 1).toString().padStart(3, '0')}`;
    }

    // Detect when the user is leaving the page (including refresh)
    window.addEventListener('beforeunload', function(event) {
        // Confirm before leaving the page
        event.returnValue = 'If you refresh this page, the ticket number will be reset.';
    });
